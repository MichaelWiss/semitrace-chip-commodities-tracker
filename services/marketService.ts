...
const fetchRealData = async (apiFunc: string): Promise<PricePoint[] | null> => {
  try {
    ...
    const response = await fetch(
        `${BASE_URL}?function=${apiFunc}&interval=monthly&apikey=${API_KEY}`,
...
export const getRiskAlerts = async (): Promise<RiskAlert[]> => {
  const allCommodities = [...RAW_SILICON, ...CRITICAL_METALS, ...PRECIOUS_METALS, ...PLATINUM_GROUP, ...RARE_EARTHS, ...SEMICONDUCTOR_ELEMENTS, ...BATTERY_MATERIALS, ...INDUSTRIAL_GASES, ...SUPERCONDUCTOR_MATERIALS, ...SPECIALTY_CHEMICALS, ...PACKAGING_MATERIALS, ...ADVANCED_MATERIALS];
